...
# GPU access: /dev/dri's render node is group-owned, but that group's GID
# is host-specific (confirmed 991 for "render" on one real host here, NOT
# the well-known video=44 that card0 uses) -- so it can't be baked into
# the image at build time. Instead, match whatever GID actually owns each
# device node right now, reusing an existing container group of that GID
# if one already exists (e.g. video=44) or creating one on the fly
# otherwise, and add abc to it. Without this, QSV/VAAPI fail with a
# permission error at runtime even though the device itself is passed
# through fine, since abc is no longer root.
